...
class EnigmaTest < Minitest::Test
  ...
  def setup
    ...
    @encrypt = Enigma.new(@file_path, @encrypted_file)
  end

  def test_enigma_is_initialized_with_certain_parameters
    assert Enigma.new(@file_path, "encrypted.txt")
  end

  def test_it_can_read_a_file
    assert_equal "Yay for crema coffee\n", @encrypt.read_message
  end
